...
import com.example.educher_child.Model.IgnoreItem;
import com.example.educher_child.Model.SortEnum;
            ...
            while (events.hasNextEvent()) {
                ...
                if (currentPackage.equals(target)) { // 本次交互开始
                    ...
                    if (prevEndEvent != null && start > 0) {
                        ...
                        if (item.mUsageTime > AppConfiguration.USAGE_TIME_MIX) item.mCount++;
                        items.add(item.copy());
                        start = 0;
                        prevEndEvent = null;
                    }
                ...
                if (!prevPackage.equals(eventPackage)) { // 包名有变化
                    if (startPoints.containsKey(prevPackage) && endPoints.containsKey(prevPackage)) {
                        ...
                        if (listItem != null) { // update list item info
                            ...
                            if (duration > AppConfiguration.USAGE_TIME_MIX) {
                                listItem.mCount++;
                            }
                        }
